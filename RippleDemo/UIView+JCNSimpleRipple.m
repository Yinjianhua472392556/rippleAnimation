...
@implementation UIView (JCNSimpleRipple)

- (void)setClickCallback:(JCNFlashButtonDidClickBlock)clickCallback {
    [self jcn_associateValue:clickCallback withKey:@selector(clickCallback)];
}

- (JCNFlashButtonDidClickBlock)clickCallback {
    return [self jcn_associatedValueForKey:@selector(clickCallback)];
}
